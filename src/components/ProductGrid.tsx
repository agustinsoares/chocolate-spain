...
const ProductCard = ({ product, compact }: { product: Product; compact: boolean }) => {
      ...
      <div className={compact ? "p-3 sm:p-6" : "p-6"}>
        <h3
          className={`font-serif font-medium text-foreground ${
            compact ? "text-base mb-1 sm:text-xl sm:mb-2" : "text-xl mb-2"
          }`}
        >
          {product.name}
        </h3>
        ...
        <a
          ...
          className={`inline-flex items-center font-sans font-medium text-primary hover:text-accent transition-colors ${
            compact ? "gap-1.5 text-xs sm:gap-2 sm:text-sm" : "gap-2 text-sm"
          }`}
        >
          <svg className={compact ? "w-3.5 h-3.5 sm:w-4 sm:h-4" : "w-4 h-4"} viewBox="0 0 24 24" fill="currentColor">
    ...
    <section id="products" className="py-20 bg-background">
      <div className="container mx-auto px-6">
        ...
        <motion.div
          ...
          className={`grid ${
            mobileColumns === 2 ? "grid-cols-2 gap-4" : "grid-cols-1 gap-8"
          } sm:grid-cols-2 sm:gap-8 lg:grid-cols-3`}
        >
          {products.map((product) => (
            <ProductCard key={product.name} product={product} compact={mobileColumns === 2} />
          ))}
        </motion.div>
      </div>
    </section>
